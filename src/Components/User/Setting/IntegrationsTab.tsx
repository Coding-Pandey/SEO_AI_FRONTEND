...
import { getProviderName } from "./ProfileServices";
...
const IntegrationsTab = ({
  ...
  const handleProviderName = async (item: any): Promise<void> => {
    setIsLoading(true);
    try {
      const response = await getProviderName(item.category);
      const { data } = response;
      if (!data.categories || Object.keys(data.categories).length === 0) {
        throw new Error("No Provider Name Found");
      }

      toast.success("Provider name fetched successfully!", {
        position: "top-right",
        autoClose: 2000,
      });
                    ...
                    return (
                      <li className="col" key={index}>
                        <div
                          ...
                          onClick={() => handleProviderName(item)}
                        >
